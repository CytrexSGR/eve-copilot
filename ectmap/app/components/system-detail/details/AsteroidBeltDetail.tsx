...
import type { SelectedObject, AsteroidBeltWithNames } from '../types';
import type { SystemDetailResponse, AsteroidBelt } from '@/lib/sde-types';
import CelestialStats from './CelestialStats';

interface AsteroidBeltDetailProps {
  data: AsteroidBelt;
  onClose: () => void;
...
export default function AsteroidBeltDetail({
  data,
  onClose,
  systemData,
  setSelectedObject,
}: AsteroidBeltDetailProps) {
  const beltWithNames = data as AsteroidBeltWithNames;
  if (!data) return null;
  ...
  return (
    <div className="absolute top-1/2 right-4 transform -translate-y-1/2 bg-gray-900 border border-gray-700 rounded-lg shadow-lg max-w-md w-80 overflow-hidden max-h-[90vh] overflow-y-auto detail-panel-scroll">
      ...
      <div className="bg-gray-800 px-4 py-3 flex justify-between items-center border-b border-gray-700 sticky top-0 z-10">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-white font-semibold text-lg">
            {beltWithNames.fullName || `Asteroid Belt ${data.orbitIndex}`}
          </h3>
        </div>
      ...
      <div className="p-4">
        <CelestialStats statistics={data.statistics} radius={data.radius} type="asteroidBelt" />
      </div>
    </div>
  );
}
