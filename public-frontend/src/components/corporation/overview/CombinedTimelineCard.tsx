/**
 * CombinedTimelineCard Component - Modern Design
 *
...
import type { TimelineResponse, TimelineDay } from '../../../types/corporation';
...
export function CombinedTimelineCard({ corpId, days }: CombinedTimelineCardProps) {
  const [timelineData, setTimelineData] = useState<TimelineResponse | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    setLoading(true);
    corpApi.getTimeline(corpId, days)
      .then((data) => {
  ...
  const mergedTimeline: TimelineDay[] = timelineData.timeline;
  ...
  const { total_kills: totalKills, total_deaths: totalDeaths, avg_daily_kills: avgKillsPerDay, avg_daily_deaths: avgDeathsPerDay } = timelineData.summary;
