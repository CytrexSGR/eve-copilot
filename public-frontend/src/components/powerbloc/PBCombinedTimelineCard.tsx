/**
 * PowerBloc CombinedTimelineCard Component - Modern Design
 *
...
interface PBCombinedTimelineCardProps {
  offensiveData: any;
  defensiveData: any;
  days: number;
}

export function PBCombinedTimelineCard({ offensiveData, defensiveData, days }: PBCombinedTimelineCardProps) {
  ...
  const mergedTimeline = Array.from(dayMap.values()).sort((a, b) => a.day.localeCompare(b.day));
  ...
  const huntingHours = offensiveData.hunting_hours || { peak_start: 0, peak_end: 0, safe_start: 0, safe_end: 0 };
  const safeDangerHours = defensiveData.safe_danger_hours || { safe_start: 0, safe_end: 0, danger_start: 0, danger_end: 0 };
  ...
  return (
    <div style={{
      ...
      <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} style={{ display: 'block' }}>
        <defs>
          <linearGradient id="pbKillGradient" x1="0%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stopColor="#3fb950" stopOpacity={0.3} />
            <stop offset="100%" stopColor="#3fb950" stopOpacity={0.05} />
          </linearGradient>
          <linearGradient id="pbDeathGradient" x1="0%" y1="0%" x2="0%" y2="100%">
        ...
        {[0, 0.25, 0.5, 0.75, 1].map(level => {
          ...
          return (
            <g key={level}>
              <line x1={padding.left} y1={y} x2={width - padding.right} y2={y} stroke="rgba(255,255,255,0.05)" strokeWidth={1} strokeDasharray={level === 0 ? '0' : '2,4'} />
              <text x={padding.left - 5} y={y + 3} fill="#8b949e" fontSize="10" textAnchor="end" fontFamily="monospace">{value}</text>
            </g>
          );
        })}
        ...
        {[0, 0.25, 0.5, 0.75, 1].map(level => {
          ...
          return (
            <text key={`right-${level}`} x={width - padding.right + 5} y={y + 3} fill="#a855f7" fontSize="10" textAnchor="start" fontFamily="monospace">{value}</text>
          );
        })}
        ...
        {mergedTimeline.filter((_, i) => i % step === 0).map((t, idx) => {
          ...
          return (
            <text key={idx} x={x} y={height - 15} fill="rgba(255,255,255,0.4)" fontSize="10" textAnchor="middle" fontFamily="monospace">{t.day.slice(5)}</text>
          );
        })}

        {/* Fill areas */}
        <path d={killFillPath} fill="url(#pbKillGradient)" />
        <path d={deathFillPath} fill="url(#pbDeathGradient)" />
        ...
        {/* Y-Axis Titles */}
        <text x={padding.left - 5} y={padding.top - 10} fill="#8b949e" fontSize="9" textAnchor="end" fontWeight="600">Kills/Deaths</text>
        <text x={width - padding.right + 5} y={padding.top - 10} fill="#a855f7" fontSize="9" textAnchor="start" fontWeight="600">Active Pilots</text>
