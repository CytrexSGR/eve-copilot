/**
 * Alliance OverviewView Component
 *
 * Combines Corporation-style overview cards with Alliance-specific intelligence panels.
 */

import { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import type { AllianceComplete } from '../../types/alliance';
import { CombinedTimelineCard } from './CombinedTimelineCard';
import {
  OffensiveCard,
  DefensiveCard,
  CapitalCard,
...
import {
  getCorpActivityHeatmap,
  getParticipationTrends,
  getBurnoutIndex,
  getAttritionTracker,
  type CorpHeatmapResponse,
  type ParticipationTrendsResponse,
  type BurnoutIndexResponse,
  type AttritionTrackerResponse,
} from '../../services/allianceApi';

interface OverviewViewProps {
  allianceId: number;
  days: number;
  data: AllianceComplete;
}

export function OverviewView({ allianceId, days, data }: OverviewViewProps) {
  const { enemies, coalition, recommendations } = data;

  const [corpHeatmap, setCorpHeatmap] = useState<CorpHeatmapResponse | null>(null);
  ...
  useEffect(() => {
    getCorpActivityHeatmap(allianceId, days).then(setCorpHeatmap).catch(() => {});
    getParticipationTrends(allianceId, Math.max(days, 14)).then(setTrends).catch(() => {});
    getBurnoutIndex(allianceId, Math.max(days, 14)).then(setBurnout).catch(() => {});
    getAttritionTracker(allianceId, Math.max(days, 30)).then(setAttrition).catch(() => {});
  }, [allianceId, days]);

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: '0.75rem' }}>
      {/* COMBINED TIMELINE */}
      <CombinedTimelineCard allianceId={allianceId} days={days} />

      {/* COMPACT SUMMARY CARDS - Responsive grid */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.75rem' }}>
        <OffensiveCard allianceId={allianceId} days={days} />
        <DefensiveCard allianceId={allianceId} days={days} />
        <CapitalCard allianceId={allianceId} days={days} />
        <PilotCard allianceId={allianceId} days={days} />
        <GeographyCard allianceId={allianceId} days={days} />
        <ActivityCard allianceId={allianceId} days={days} />
        <HuntingCard allianceId={allianceId} days={days} />
      </div>
              ...
              {attrition.destinations.length > 0 && (
                <div style={{ padding: '0 0.5rem 0.4rem' }}>
                  ...
                  {attrition.destinations.slice(0, 5).map((dest, i) => (
                    <Link key={dest.alliance_id} to={`/alliance/${dest.alliance_id}`} style={{ textDecoration: 'none' }}>
                      <div style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: '0.4rem',
                        ...
                        <img
                          src={`https://images.evetech.net/alliances/${dest.alliance_id}/logo?size=32`}
                          alt=""
      ...
      {/* CORP ACTIVITY HEATMAP */}
      {corpHeatmap && corpHeatmap.corps.length > 0 && (
        <div style={{
          background: 'rgba(0,0,0,0.3)',
          borderRadius: '6px',
          border: '1px solid rgba(255,255,255,0.08)',
          ...
          <div style={{
            ...
            <span style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.4)' }}>
              {corpHeatmap.corps.length} corps • {days}d
            </span>
          </div>
          ...
          <div style={{ padding: '0 0.5rem 0.5rem', maxHeight: '260px', overflowY: 'auto' }}>
            {corpHeatmap.corps.map(corp => {
              const maxHour = Math.max(...corp.hours, 1);
              const peakHour = corp.hours.indexOf(Math.max(...corp.hours));
              return (
                <div key={corp.corp_id} style={{
                  ...
                  <div style={{
                    ...
                    paddingRight: '0.4rem',
                  }} title={`${corp.corp_name} [${corp.ticker}] - Peak: ${peakHour}:00 UTC`}>
                    <span style={{ color: 'rgba(255,255,255,0.4)' }}>[{corp.ticker}]</span> {corp.corp_name}
                  </div>
                  <div style={{ flex: 1, display: 'flex', gap: '1px' }}>
                    {corp.hours.map((val, h) => {
                        ...
                        <div
                          ...
                          style={{
                            ...
                            background: val === 0
                              ...
                              : intensity > 0.7
                                ? `rgba(0,188,212,${0.3 + intensity * 0.7})`
                                : intensity > 0.3
                                  ? `rgba(0,188,212,${0.15 + intensity * 0.5})`
                                  : `rgba(0,188,212,${0.05 + intensity * 0.3})`,
                          }}
                          title={`${corp.corp_name} ${h}:00 - ${val} activity`}
                        />
                      );
                    })}
                  </div>
      ...
      {/* ALLIANCE-SPECIFIC: Top Enemies, Coalition Allies, Recommendations */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.75rem' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.4)' }}>
              {enemies.top_enemies?.length || 0} enemies
            </span>
          </div>
          <div style={{ padding: '0.25rem', maxHeight: '280px', overflowY: 'auto' }}>
            {(enemies.top_enemies || []).slice(0, 10).map((enemy, i) => (
              <Link
                key={i}
                to={`/alliance/${enemy.alliance_id}`}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.4)' }}>
              {coalition.detected_allies?.length || 0} allies
            </span>
          </div>
          <div style={{ padding: '0.25rem', maxHeight: '280px', overflowY: 'auto' }}>
            {(coalition.detected_allies || []).slice(0, 10).map((ally, i) => (
              <Link
                key={i}
              ...
              >
                <div style={{
                  ...
                  <span style={{ fontSize: '0.7rem', fontWeight: 700, color: '#3fb950', fontFamily: 'monospace' }}>
                    {ally.joint_kills}
                  </span>
                  <span style={{ fontSize: '0.55rem', color: 'rgba(255,255,255,0.4)' }}>joint</span>
                </div>
              </Link>
            ))}
          </div>
        ...
        <div style={{
          ...
          <div style={{ padding: '0.25rem', maxHeight: '280px', overflowY: 'auto' }}>
            {(recommendations || []).slice(0, 5).map((rec, i) => {
              const catColor = rec.category === 'avoid' ? '#ff4444'
                : rec.category === 'attack' ? '#3fb950'
                : rec.category === 'fit' ? '#58a6ff'
                : '#a855f7';
              return (
                <div key={i} style={{
                  display: 'flex',
