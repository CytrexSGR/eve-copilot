/**
 * Alliance OffensiveCard - Compact Summary Card
 */
...
export function OffensiveCard({ allianceId, days }: OffensiveCardProps) {
    ...
    <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginBottom: '0.4rem' }}>
        ...
        <div>
          ...
          <div style={{ fontSize: '1.2rem', fontWeight: 700, color: '#3fb950', fontFamily: 'monospace' }}>
            {(parseFloat(summary.isk_destroyed || '0') / 1e12).toFixed(1)}T
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.55rem', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '0.3rem' }}>
        <div>
          ...
          <span style={{ color: '#fff', fontWeight: 600 }}>{(data.top_victims?.[0] as any)?.alliance_name || (data.top_victims?.[0] as any)?.corporation_name || 'N/A'}</span>
        </div>
        <div>
