...
class RedisStateManager:
    ...
    # =========================================================
    # R2Z2 Sequence Tracking
    # =========================================================

    _R2Z2_SEQUENCE_KEY = "zkill:r2z2_sequence"

    def save_r2z2_sequence(self, sequence_id: int):
        """
        Persist the last successfully processed R2Z2 sequence number.

        Called after each killmail is handed off to process_live_kill().
        Survives restarts — on startup, we resume from this sequence.

        Args:
            sequence_id: The sequence number that was just processed
        """
        try:
            self.redis_client.set(self._R2Z2_SEQUENCE_KEY, str(sequence_id))
        except redis.RedisError as e:
            print(f"[WARNING] Redis error saving R2Z2 sequence: {e}")

    def get_r2z2_sequence(self) -> Optional[int]:
        """
        Get the last processed R2Z2 sequence number.

        Returns:
            Last processed sequence ID, or None if never run before
        """
        try:
            value = self.redis_client.get(self._R2Z2_SEQUENCE_KEY)
