"""Market repository - data access layer with L1/L2/L3 caching."""
import logging
...
from app.models import MarketPrice, CacheStats, PriceSource, JITA_REGION_ID
from app.services.cache import MarketCache
from app.services.hot_items import get_hot_items

logger = logging.getLogger(__name__)


class MarketRepository:
    """Data access for market prices (L2 PostgreSQL cache)."""
    ...
    def bulk_upsert_prices(self, prices: List[MarketPrice]) -> int:
        ...
        try:
            ...
            logger.error(f"Failed to bulk upsert prices: {e}")
            raise
                ...
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    if row and row["total_items"] > 0:
                        ...
                        if newest_entry:
                            ...
                            is_stale = cache_age_seconds > 3600
    ...
    def get_price(self, type_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to get price for type_id {type_id}: {e}")
            raise
        ...
        try:
            with self.db.connection() as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    if not type_ids:
                        cur.execute(
                            "SELECT * FROM market_prices_cache WHERE adjusted_price > 0"
                        )
                    else:
                        cur.execute(
                            "SELECT * FROM market_prices_cache WHERE type_id = ANY(%s)",
                            (list(type_ids),)
                        )
                    return [dict(row) for row in cur.fetchall()]
        except Exception as e:
            logger.error(f"Failed to get prices bulk: {e}")
            raise
...
class UnifiedMarketRepository:
    ...
    def get_price(
        ...
        """Get price using L1 -> L2 -> L3 fallback chain."""
        # L1: Try Redis first
    ...
    def get_prices(
        ...
        # L2: Batch fetch from PostgreSQL
        ...
        try:
            with self.db.connection() as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    cur.execute(
                        """
                        SELECT type_id, lowest_sell, highest_buy, updated_at
                        FROM market_prices
    ...
    def _fetch_from_esi(
        ...
        return self.esi.fetch_price(type_id, region_id)
    ...
    def _fetch_bulk_from_esi(
        ...
        return self.esi.fetch_prices_bulk(type_ids, region_id)
                ...
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        INSERT INTO market_prices
                            (type_id, region_id, lowest_sell, highest_buy, updated_at)
                        VALUES (%s, %s, %s, %s, %s)
