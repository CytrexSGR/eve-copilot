...
class SkillPlannerService:
    ...
    def get_skill_info(self, request: Request, skill_type_ids: List[int]) -> Dict[int, dict]:
    ...
    def get_character_skill_levels(self, request: Request, character_id: int) -> Dict[int, int]:
        ...
        if not result:
            return {}
        return {s.skill_id: s.trained_level or s.level for s in result.skills}
    ...
    def calculate_plan(
        ...
        skill_info = self.get_skill_info(request, skill_ids)

        # Get character's current levels
        char_levels = self.get_character_skill_levels(request, character_id)
