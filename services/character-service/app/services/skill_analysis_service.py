...
class SkillAnalysisService:
    ...
    def get_character_profile(self, request: Request, character_id: int) -> Dict[str, Any]:
    ...
    def get_team_overview(self, request: Request) -> Dict[str, Any]:
    ...
    def get_skill_gaps(self, request: Request) -> List[Dict[str, Any]]:
    ...
    def get_skill_comparison(self, request: Request) -> List[Dict[str, Any]]:
    ...
    def get_unique_capabilities(self, request: Request) -> List[Dict[str, Any]]:
    ...
    def get_recent_progress(self, request: Request, days: int = 30) -> List[Dict[str, Any]]:
    ...
    def prepare_analysis_input(
        ...
        data = {
            ...
            "team_overview": self.get_team_overview(request),
        }

        if analysis_type == AnalysisType.INDIVIDUAL_ASSESSMENT:
            if character_ids:
                data["characters"] = [
                    self.get_character_profile(request, cid) for cid in character_ids
                ]
            else:
                # Get all characters
                db = request.app.state.db
                ...
                data["characters"] = [
                    self.get_character_profile(request, cid) for cid in all_ids
                ]

        elif analysis_type == AnalysisType.TEAM_COMPOSITION:
            data["unique_capabilities"] = self.get_unique_capabilities(request)
            data["skill_comparison"] = self.get_skill_comparison(request)

        elif analysis_type == AnalysisType.TRAINING_PRIORITIES:
            data["skill_gaps"] = self.get_skill_gaps(request)
            data["skill_comparison"] = self.get_skill_comparison(request)
            data["recent_progress"] = self.get_recent_progress(request)
            ...
            data["progress"] = self.get_recent_progress(request, days)
            data["skill_gaps"] = self.get_skill_gaps(request)
    ...
    def create_snapshot(self, request: Request, character_id: int, snapshot_date: Optional[date] = None) -> int:
    ...
    def create_all_snapshots(self, request: Request, snapshot_date: Optional[date] = None) -> List[Dict[str, int]]:
    ...
    def calculate_progress(self, request: Request, character_id: int, progress_date: Optional[date] = None) -> int:
    ...
    def mark_recommendation_completed(self, request: Request, recommendation_id: int) -> bool:
