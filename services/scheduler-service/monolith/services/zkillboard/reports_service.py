...
class ZKillboardReportsService(
    ...
    async def detect_coalitions(self, days: int = 7, min_fights_together: int = 5, minutes: int = None) -> Dict:
        ...
        MIN_TOGETHER_RATIO = 3.0  # Must fight together 3x more than against
        MIN_SOLO_ACTIVITY = 1000  # Solo alliances with this much activity = own Power Bloc

        try:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    # Step 1: Get alliance pairs from pre-computed tables (90-day data, filtered)
                    cur.execute("""
                        SELECT
                            t.alliance_a,
                            t.alliance_b,
                    ...
                    def safe_union(x, y):
                        """Union only if no members are enemies"""
                        px, py = find(x), find(y)
                        if px == py:
                    ...
                    # Filter to true coalition partners using friend/enemy ratio
                    coalition_pairs = []
                    for row in fight_pairs:
                        ...
                        # Skip if they fight against each other more than together
                        if against > 0 and together / against < MIN_TOGETHER_RATIO:
                            continue
