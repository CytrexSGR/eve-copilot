...
class WorkflowService:
    ...
    def create_job(
        ...
        try:
            ...
            if not job_id:
                raise WorkflowServiceError("Failed to create job - no ID returned")
            ...
            logger.error(f"Repository error creating job: {e}")
    ...
    def get_jobs(
        ...
        try:
            ...
            logger.error(f"Repository error getting jobs: {e}")
            raise WorkflowServiceError(str(e))
    ...
    def update_job(
        ...
        try:
            ...
            if not success:
                raise WorkflowServiceError("No updates provided")
            ...
            logger.error(f"Repository error updating job: {e}")
            raise WorkflowServiceError(str(e))
    ...
    def get_job_materials(self, job_id: int) -> List[Dict[str, Any]]:
        ...
        """
        try:
            return self.repo.get_job_materials(job_id)
        except WorkflowRepositoryError as e:
            logger.error(f"Repository error getting job materials: {e}")
            raise WorkflowServiceError(str(e))
